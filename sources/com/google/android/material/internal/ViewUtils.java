...
public class ViewUtils {
    ...
    public interface OnApplyWindowInsetsListener {
        ad onApplyWindowInsets(View view, ad adVar, RelativePadding relativePadding);
    }
    ...
    public static class RelativePadding {
        ...
        public void applyToView(View view) {
            v.b(view, this.start, this.top, this.end, this.bottom);
        }
    }
    ...
    public static void doOnApplyWindowInsets(View view, AttributeSet attributeSet, int i, int i2, final OnApplyWindowInsetsListener onApplyWindowInsetsListener) {
        ...
        doOnApplyWindowInsets(view, new OnApplyWindowInsetsListener() {
            public ad onApplyWindowInsets(View view, ad adVar, RelativePadding relativePadding) {
                if (z) {
                    relativePadding.bottom += adVar.d();
                }
                boolean isLayoutRtl = ViewUtils.isLayoutRtl(view);
                if (z2) {
                    if (isLayoutRtl) {
                        relativePadding.end += adVar.a();
                    } else {
                        relativePadding.start += adVar.a();
                    }
                }
                if (z3) {
                    if (isLayoutRtl) {
                        relativePadding.start += adVar.c();
                    } else {
                        relativePadding.end += adVar.c();
                    }
                }
                relativePadding.applyToView(view);
                OnApplyWindowInsetsListener onApplyWindowInsetsListener = onApplyWindowInsetsListener;
                return onApplyWindowInsetsListener != null ? onApplyWindowInsetsListener.onApplyWindowInsets(view, adVar, relativePadding) : adVar;
            }
        });
    }

    public static void doOnApplyWindowInsets(View view, final OnApplyWindowInsetsListener onApplyWindowInsetsListener) {
        final RelativePadding relativePadding = new RelativePadding(v.j(view), view.getPaddingTop(), v.k(view), view.getPaddingBottom());
        v.a(view, (q) new q() {
            public ad onApplyWindowInsets(View view, ad adVar) {
                return onApplyWindowInsetsListener.onApplyWindowInsets(view, adVar, new RelativePadding(relativePadding));
            }
        });
