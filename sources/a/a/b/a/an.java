...
public class an extends b {
    public static LruCache<String, an> q = new LruCache<>(5);
    public float[] r;
    public Mesh s;

    public an(Resources resources, Context context) {
        super(resources, t.a("darkchannel_2"), context);
    }

    public static an a(Resources resources, Context context) {
        an anVar = q.get(Thread.currentThread().getName());
        if (anVar == null) {
            anVar = new an(resources, context);
            anVar.a();
            q.put(Thread.currentThread().getName(), anVar);
        }
        anVar.a(context);
        return anVar;
    }
