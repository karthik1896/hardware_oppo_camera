...
public class g implements Interpolator {
    ...
    private void a(Path path) {
        ...
        int min = Math.min(ApsAdapterConstant.LOG_INTERVAL, ((int) (length / 0.002f)) + 1);
        if (min > 0) {
            this.f1329a = new float[min];
            this.f1330b = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, (float[]) null);
                this.f1329a[i2] = fArr[0];
                this.f1330b[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f1329a[0])) <= 1.0E-5d && ((double) Math.abs(this.f1330b[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f1329a[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f1330b[i3] - 1.0f)) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f1329a;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb.append(this.f1329a[0]);
            sb.append(",");
            sb.append(this.f1330b[0]);
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f1329a[i6]);
            sb.append(",");
            sb.append(this.f1330b[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f1329a.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f1329a[i2]) {
                length = i2;
